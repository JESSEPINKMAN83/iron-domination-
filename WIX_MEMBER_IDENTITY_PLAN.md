...
## Phase 5 — Per-member stats (in progress)

Aggregates only. Counters and timestamps per member, never a per-match log tied to
a person: enough to answer "who comes back, and who wins" without turning the game
into a record of what an identified player did on a given evening. Single player is
never reported — an account is a multiplayer requirement, and solo players stay
entirely anonymous.

### Wix side — done

`IronDominionPlayers` collection created (backend-only: ADMIN for read, insert,
update and remove — nothing is reachable from the site):

| Field | Type |
|---|---|
| `memberId` | Text |
| `nickname` | Text |
| `firstSeen` / `lastSeen` | Datetime |
| `mpMatches` / `mpWins` / `mpLosses` | Number |
| `totalPlayMinutes` | Number |
| `bestRankAceShare` | Number |

### Game side — done

`src/identity/playerStats.ts` decides what to report. `playerStatUpdate()` returns
an update only for a **finished multiplayer match played by a signed-in member**,
with play minutes rounded to one decimal and the ace share clamped to 0–1. Seven
tests pin those rules, including that single player and guests report nothing.

### Remaining

1. **Worker** — accept `kind: 'player-stat'` in `serverless/wix-backoffice.mjs`,
   validate it the way telemetry is validated, and forward to the CMS endpoint. It
   needs no Wix API key, exactly like telemetry.
2. **Velo** — a `player-stat` branch in `post_ironDominionSubmission` that queries
   `IronDominionPlayers` by `memberId` and inserts or updates with `suppressAuth`:
   increment `mpMatches`, and `mpWins` or `mpLosses`; add `totalPlayMinutes`; raise
   `bestRankAceShare` only when the new value is higher; set `firstSeen` on insert
   and `lastSeen` always.
3. **Hook it up** — call `playerStatUpdate()` where `match-end` telemetry is sent
   (`src/telemetry.ts`) and post it through the existing backoffice client.
4. **Dashboard** — a Members card: total members, new this week, matches per
   member, win rate. Same Aria workflow as the existing cards.

The Velo edit is the only step that cannot be done from this repo; it is edited
directly in the Wix editor's code panel, as the telemetry mapping was.
