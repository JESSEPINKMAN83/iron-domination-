...
import { createBuildingStudioScene, frameBuildingPortrait, setCommandPortrait } from '../render/buildingPortraits';
import { createUnitPreview as createBuildingPreview, disposeUnitPreview as disposeBuildingPreview, UNIT_PORTRAIT_MODELS } from '../render/unitView';
const BUILDING_PORTRAIT_KINDS = Object.keys(UNIT_PORTRAIT_MODELS);
const buildingPortraitLabel = (key: string) => UNIT_PORTRAIT_MODELS[key].label;

if (!import.meta.env.DEV) throw new Error('The unit studio is only available in local development.');
